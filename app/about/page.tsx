...
export const metadata = pageMetadata({
  ...
  description: "Project Monet is a creator-led, Instagram-only marketing and management agency for founders and businesses worldwide, built around Funnel-First strategy.",
  path,
});

export default function Page() {
  const crumbs = [{ label: "Home", href: "/" }, { label: "About Project Monet", href: path }];
  const aboutSchema = { "@context": "https://schema.org", "@type": "AboutPage", name: "About Project Monet", url: absoluteUrl(path), description: "Project Monet is a creator-led, Instagram-only marketing and management agency for founders and businesses worldwide.", mainEntity: { "@type": "Organization", name: "Project Monet", url: absoluteUrl("/"), areaServed: "Worldwide" } };
  return (
    <>
      <main>
        <PageHero eyebrow="About Project Monet" title="We learned Instagram by doing it." intro="Project Monet is a creator-led, Instagram-only marketing and management agency for founders and businesses worldwide. We work remotely and build strategy before random posting." path={path} currentLabel="About Project Monet" />
        ...
        <section className="editorial-section warm-section"><div className="page-shell editorial-split"><div><p className="eyebrow">Creator-led, business-first</p><h2>What happens after the view matters.</h2></div><div className="prose-large"><p>We care about reach. We also care about profile visits, follows, engagement, DMs, leads, and business actions.</p><p>Standard Management is the controlled-management path without a performance guarantee. Viral Mandate is the qualified six-month path for accounts ready to pursue an account-specific organic Instagram performance result under signed terms.</p><p>A Reel with fewer views can be more valuable when the right people see it. Growth is not only knowing what to post. It is also knowing what not to post.</p><Link className="text-link" href="/instagram-management-services">Compare the two ways to work with Project Monet <ArrowRight aria-hidden="true" /></Link></div></div></section>
