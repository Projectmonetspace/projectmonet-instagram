...
export const metadata = pageMetadata({
  ...
  description: "Plain-language Instagram marketing resources for businesses: costs, Reels strategy, content strategy, profile optimization, Instagram SEO, leads, and growth problems.",
  path,
});
  ...
  return (
    <>
      <main>
        ...
        <section className="resource-index dark-section"><div className="page-shell"><p className="eyebrow">Eight practical guides</p><div className="resource-card-grid">{resourceArticles.map((article, index) => <Link key={article.slug} href={`/resources/${article.slug}`}><span>{String(index + 1).padStart(2, "0")}</span><small>{article.eyebrow}</small><h2>{article.title}</h2><p>{article.description}</p><i>{article.readTime}<ArrowRight aria-hidden="true" /></i></Link>)}</div></div></section>
